...
import { fetchServicios } from './serviciosAPI';
import { fetchCategoriasServicios } from './categoriaServiciosAPI';
          ...
          <div className="services-grid-catalog-new">
            {services.slice(currentPage * 3, currentPage * 3 + 3).map(service => (
              <div key={service.id} className="service-card-catalog-new">
                ...
                <p>Categoría: {categories.find(cat => cat.id === service.categoria_id)?.nombre || 'Sin categoría'}</p>
