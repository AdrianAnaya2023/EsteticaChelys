...
const SpecialOffers = ({ onClose }) => {
  ...
  if (loading) {
    return null; // No mostrar nada mientras se cargan las promociones
  }

  if (error || offers.length === 0) {
    return null; // No mostrar nada si hay error o no hay promociones
  }

  // Función para formatear la fecha sin la hora
  const formatDate = (dateString) => {
    const date = new Date(dateString);
    return date.toLocaleDateString(); // Formatea la fecha solo mostrando día, mes y año
  };
        ...
        {offers.length > 0 && (
          <div key={offers[currentOfferIndex].id} className="offer-cardOfertillas">
            ...
            <div className="offer-infoOfertillas">
              ...
              <p className="offer-end-dateOfertillas">Finaliza el: {formatDate(offers[currentOfferIndex].fecha_fin)}</p> {/* Mostrar solo la fecha formateada */}
            </div>
          </div>
        )}
