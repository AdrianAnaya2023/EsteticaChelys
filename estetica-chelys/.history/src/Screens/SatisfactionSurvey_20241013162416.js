        ...
        <div className="form-content-encuesta">
          ...
          <form onSubmit={handleSubmit}>
            {questions.map((question) => (
              <div key={question.id} className="question-block-encuesta"> {/* Clase para margen */}
                <label htmlFor={`question-${question.id}`} className="label-encuesta">{question.pregunta}</label>
                <select
                  id={`question-${question.id}`}
                  name={`question-${question.id}`}
                  value={answers[question.id] || ''}
                  onChange={(e) => handleAnswerChange(question.id, e.target.value)}
                  className="select-encuesta" // Clase específica para el select
                >
