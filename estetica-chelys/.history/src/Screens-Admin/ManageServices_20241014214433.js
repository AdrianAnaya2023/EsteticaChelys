...
const ManageServices = ({ onClose }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (isServicesView) {
        if (!newItem.titulo || !newItem.descripcion || !newItem.imagen || !newItem.categoria_id) {
          return toast.error('Todos los campos del servicio son obligatorios.');
        }
  
        // Notificación de lo que se está enviando al backend
        toast.info(`Enviando servicio al backend: Título: ${newItem.titulo}, Descripción: ${newItem.descripcion}, Imagen: ${newItem.imagen}, Categoría ID: ${newItem.categoria_id}`);
        ...
        // Notificación de lo que se está enviando al backend
        toast.info(`Enviando categoría al backend: Nombre: ${newItem.nombre}, Descripción: ${newItem.descripcion}, Imagen: ${newItem.imagen}, Categoría ID: ${newItem.categoria_id}`);
