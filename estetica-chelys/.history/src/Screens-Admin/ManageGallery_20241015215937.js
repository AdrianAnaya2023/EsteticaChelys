...
const ManageGallery = ({ onClose }) => {
  ...
  // Manejar los cambios en los archivos
  const handleFileChange = async (e, isBefore, isCategory = false) => {
    const file = e.target.files[0];
    if (!file) return;

    const filePath = isCategory ? `categorias/${file.name}` : `galeria/${file.name}`;
    ...
    uploadTask.on(
      ...
      async () => {
        ...
        if (isBefore) {
          ...
          setNewItem((prev) => ({ ...prev, imagenCategoria: downloadURL }));
        } else {
            ...
            {isGalleryView ? (
              ...
              <>
                ...
                <input
                  type="file"
                  id="imagenFile"
                  onChange={(e) => handleFileChange(e, false, true)} // Imagen de categoría
                  className="input-GaleriaAdmin"
                />
                {uploadProgress > 0 && (
                  <div className="progress-bar">
