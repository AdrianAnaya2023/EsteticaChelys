...
const ManageGallery = ({ onClose }) => {
  ...
  const [fileCategory, setFileCategory] = useState(null); // Estado para imagen de categoría
  ...
  const handleFileChange = async (e, type) => {
    ...
    try {
      const filePath = type === 'category' ? `categorias/${file.name}` : `galeria/${file.name}`;
      const imageUrl = await uploadFileToFirebase(file, filePath);
      if (type === 'foto_antes') {
        setNewItem((prev) => ({ ...prev, foto_antes: imageUrl }));
      } else if (type === 'foto_despues') {
        setNewItem((prev) => ({ ...prev, foto_despues: imageUrl }));
      } else if (type === 'category') {
        setNewItem((prev) => ({ ...prev, imagenCategoria: imageUrl }));
      }
