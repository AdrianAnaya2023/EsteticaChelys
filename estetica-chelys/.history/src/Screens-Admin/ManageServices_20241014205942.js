...
const ManageServices = ({ onClose }) => {
  ...
  const [newItem, setNewItem] = useState({
    ...
    descripcion: '',
    imagen: '',
    categoria_id: '', // Verificamos que el valor inicial esté vacío
    nombre: '',
  });
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setCategories(loadedCategories || []); // Aseguramos que se carguen correctamente
      } catch (error) {
  ...
  const handleSave = async () => {
    console.log('Datos del servicio antes de guardar:', newItem); // Comprobar qué datos se están enviando
    try {
      if (isServicesView) {
        // Verificamos si todos los campos obligatorios están llenos
        ...
        // Verificación para las categorías
      ...
      console.error('Error al guardar el servicio:', error); // Mostrar el error en la consola
