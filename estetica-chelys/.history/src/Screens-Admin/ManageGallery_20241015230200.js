...
const ManageGallery = ({ onClose }) => {
  ...
  const openModal = (item = null, isEditing = false) => {
    ...
    setCurrentItem(item);
    setNewItem(item ? { ...item } : { foto_antes: '', foto_despues: '', categoria_id: null, nombre: '', descripcion: '', imagenCategoria: '' });
  };
  ...
  const handleFileChange = async (e, field) => {
    ...
    try {
      ...
      setNewItem((prev) => ({
        ...prev,
        [field]: imageUrl, // Asigna la URL de la imagen a la propiedad correspondiente
      }));
            ...
            {isGalleryView ? (
              ...
              <>
                ...
                <input
                  type="file"
                  id="imagenFile"
                  onChange={(e) => handleFileChange(e, 'imagenCategoria')}
                  className="input-GaleriaAdmin"
                />
                {uploadProgress > 0 && (
                  <div className="progress-bar">
