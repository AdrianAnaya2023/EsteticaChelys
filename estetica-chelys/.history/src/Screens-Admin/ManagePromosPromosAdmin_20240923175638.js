...
const ManagePromosPromosAdmin = () => {
  ...
  useEffect(() => {
    const loadPromos = async () => {
      try {
        setIsLoading(true); // Empieza cargando
        const data = await fetchPromos();
        if (Array.isArray(data)) {
          setPromos(data);
        } else {
          console.error("Datos inesperados recibidos de la API", data);
  ...
  // Abrir modal para agregar o editar promoción
  ...
  // Guardar promoción (crear o actualizar)
  const handleSave = async () => {
    if (!currentPromo.foto || !currentPromo.titulo || !currentPromo.descripcion || !currentPromo.fecha_fin) {
          ...
          <tbody>
            {promos.map(promo => (
              <tr key={promo.id}>
                ...
                <td>{new Date(promo.fecha_fin).toLocaleDateString()}</td> {/* Formatear fecha */}
                <td>
