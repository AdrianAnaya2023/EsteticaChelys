...
import {
  fetchCategoriasServices,
  createCategoriaService,
  updateCategoriaService,
  deleteCategoriaService,
} from './categoriaServiciosAPI';
import { ref, uploadBytesResumable, getDownloadURL } from 'firebase/storage';
import { storage } from '../firebaseConfig.js'; // Asegúrate de tener la configuración de Firebase correcta
...
const ManageServices = ({ onClose }) => {
  ...
  const handleSave = async () => {
    try {
      if (isEditing) {
        if (isServicesView) {
          const updatedService = await updateService(currentItem.id, newItem);
          setServices(
          ...
          const updatedCategory = await updateCategoriaService(
            currentItem.id,
            newItem
          );
          setCategories(
        ...
        if (isServicesView) {
          ...
          const createdCategory = await createCategoriaService(newItem);
  ...
  return (
    <div className="manage-services-container">
      ...
      <h1 className="title">{isServicesView ? 'Servicios' : 'Categorías de Servicios'}</h1>
      ...
      {isModalOpen && (
        <div className="modal">
          <div className="modal-content">
            ...
            <label className="modal-label" htmlFor="imagen">Imagen (URL)</label>
            <input
              type="file"
