  ...
  const handleSave = async () => {
    try {
      ...
      if (!newItem.titulo || !newItem.descripcion || !newItem.categoria_id || !newItem.imagen) {
        toast.error('Todos los campos del servicio son obligatorios.');
        return;
      }

      let response;
      if (isEditing) {
        response = await updateServicio(currentItem.id, newItem);
        setServices(services.map(serv => (serv.id === currentItem.id ? response : serv)));
        toast.success('Servicio actualizado con éxito');
      } else {
        response = await createServicio(newItem);
      ...
      console.error('Error al guardar el servicio:', error); // Log del error en la consola
