...
const ModifyHomeFooter = ({ onClose }) => {
  ...
  const handleFileChange = (e, isHomeImage = true) => {
    const file = e.target.files[0];
    if (!file) return;

    const filePath = isHomeImage ? `home/${file.name}` : `footer/${file.name}`;
  ...
  // Función faltante para manejar cambios en las fotos del local
