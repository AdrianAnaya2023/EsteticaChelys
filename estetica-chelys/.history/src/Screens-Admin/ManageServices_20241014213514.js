...
const ManageServices = ({ onClose }) => {
  ...
  const handleSave = async () => {
    console.log('Datos actuales del nuevo servicio:', newItem); // Verificar datos antes de enviar
    ...
    try {
      if (isServicesView) {
        ...
        console.log('Datos validados para el servicio:', newItem); // Verificar datos validados
        ...
        if (isEditing) {
          ...
          console.log('Enviando solicitud de creación de servicio...');
          const createdService = await createServicio(newItem);
          console.log('Respuesta del servidor:', createdService); // Verifica la respuesta del servidor
