...
const ManageGallery = ({ onClose }) => {
  ...
  const openModal = (item = null, isEditing = false) => {
    ...
    setFileCategory(null); // Reiniciar archivo de imagen
  };
  ...
  const handleFileChange = async (e, isCategory = false) => {
    ...
    try {
      ...
      setNewItem((prev) => ({
        ...prev,
        [isCategory ? 'imagenCategoria' : 'foto_antes']: imageUrl, // Asigna la URL de la imagen
      }));
