...
const handleAxiosError = (error, defaultMessage) => {
  if (error.response) {
    return { error: `Error del servidor: ${error.response.data.message || defaultMessage}` };
  } else if (error.request) {
    return { error: 'No se recibió respuesta del servidor. Verifique su conexión.' };
  } else {
    return { error: `Error en la solicitud: ${error.message}` };
  }
};
